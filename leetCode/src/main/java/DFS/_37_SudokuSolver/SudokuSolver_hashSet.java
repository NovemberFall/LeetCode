                ...
                if (board[i][j] == '.') {
                    ...
                    for (char num = '1'; num <= '9'; num++) {
                        if (isValid(board, i, j, num)) {
                            ...
                            if (solve(board)) {//if it's true that means we have a solved board, so you just keep returning
                                return true;
                            }
                            board[i][j] = '.';//If our board isn't solved, backtrack and try the next number
                        }
                    }
                    return false;
